...
router.post('/api/qinglongMessage', async (req, res) => {
    try {
        ...
        // M麦基脚本日志判断
        if ((message.includes('export') || message.includes('活动地址:')) && spyIsValidEnable) {
    const httpsPattern = /(https:\/\/[A-Za-z0-9\-\._~:\/\?#\[\]@!$&'\*\+,%;\=]*)(?!shop\.m\.jd\.com)/;
    const urlMatch = message.match(httpsPattern);
    
    if (urlMatch) {
        const url = urlMatch[0];
        // 获取活动ID
        const activityId = getQueryString(url, 'activityId') || getQueryString(url, 'id') ||
                          getQueryString(url, 'giftId') || getQueryString(url, 'actId') ||
                          getQueryString(url, 'code') || getQueryString(url, 'a');
        // 当前时间
                    ...
                    if (title == 'M购物车锦鲤' && /已经开奖/.test(message)) { // 购物车开奖标记
                        ...
                        if (!actCron) {
                            ...
                            if (rlt) {
                                ...
                                if (title == 'M加购有礼' || title == '加购有礼beta' && /明日再来/.test(message)) { // 明日加购
